#!/usr/bin/env bash
# .fork/audit.sh — periodic fork-health check (see Fork Maintenance in your AGENTS.md).
#
...
if [ -z "$ptr" ]; then
	...
	# pointer file. The unport-cancellation walk lives in lib.sh
	# (effective_port_trailer_sha, shared with port.sh's port-time refusal); it prints
	# "<containing-commit> <trailer-sha>" or nothing.
...
# afterwards. TWO numbers per pin, deliberately split:
#   hidden upstream — what the pin SUPPRESSES (base vs the audited upstream):
#                     the security-relevant number; it should only grow for
#                     reasons you can name.
#   fork intent     — your own deliberate edits to the pinned path (base vs HEAD).
printf '\n== merge=ours divergence (what each pin hides vs the ported upstream) ==\n'
mo_any=0
while IFS= read -r p; do
	[ -z "$p" ] && continue
	mo_any=1
	mo_hidden="$(git diff --shortstat "$base" "$upstream" -- "$p" 2>/dev/null || true)"
	mo_intent="$(git diff --shortstat "$base" -- "$p" 2>/dev/null || true)"
	if [ -z "$mo_hidden" ] && [ -z "$mo_intent" ]; then
		printf '  %s: in sync with the ported upstream\n' "$p"
		continue
	fi
	printf '  %s:\n' "$p"
	if [ -n "$mo_hidden" ]; then
		printf '    hidden upstream:%s\n' "$mo_hidden"
		printf '      inspect: git diff %s %s -- %s\n' "$(git rev-parse --short "$base")" "$(git rev-parse --short "$upstream")" "$p"
	else
		printf '    hidden upstream: nothing suppressed\n'
	fi
	if [ -n "$mo_intent" ]; then
		printf '    fork intent:%s\n' "$mo_intent"
		printf '      inspect: git diff %s -- %s\n' "$(git rev-parse --short "$base")" "$p"
	else
		printf '    fork intent: no deliberate edits\n'
	fi
...
# 1b. entries with no remaining fork delta (path-level tripwire) ----------------
# LOW SPECIFICITY, never proof: "no fork delta remains anywhere in this entry's
# anchor paths" — NOT entry-level loss (an anchor overlapping ANOTHER entry's
# delta keeps it green). Entries with only Symbols anchors are skipped — symbol
# paths alone are not delta anchors. The same per-entry computation verify.sh
# warns on at gate time; here it reads against the audit base.
printf '\n== entries with no remaining fork delta (path-level tripwire) ==\n'
tw_any=0 # entries seen at all
tw_hits=0
tw_check() { # reads $theading/$ttouches; prints a NO DELTA line on a hit
	local tok
	local toks=()
	[ -z "$theading" ] && return 0
	tw_any=1
	...
	while IFS= read -r tok; do
		[ -n "$tok" ] && toks+=("$tok")
	done < <(split_field "$ttouches")
	[ "${#toks[@]}" -eq 0 ] && return 0
	if git diff --quiet "$base" HEAD -- "${toks[@]}" 2>/dev/null; then
		printf '  NO DELTA: %s — customization gone, or upstream now ships it: drift-check; retire with drop-change if superseded\n' "$theading"
		tw_hits=1
	fi
	return 0
}
if [ -f "$changes" ]; then
	...
	while IFS= read -r line || [ -n "$line" ]; do
		...
		"## "*)
			...
			ttouches=""
			;;
		[Tt]ouches:*) [ -n "$theading" ] && ttouches="${line#*:}" ;;
		esac
	done <"$changes"
	tw_check
fi
...
if [ -n "$up_ids" ]; then
	while IFS= read -r sha; do
		...
		is_upstream_unport_commit "$sha" && continue # skip unports: a revert is the INVERSE of an upstream diff and can patch-id-collide (false "reimplemented")
