...
# (see Fork Maintenance in your AGENTS.md).
#
...
#        conflict-context.sh <from>..<to> <file>  (a `port.sh range` catch-up — the span's diff)
# The explicit form is preferred: it avoids depending on MERGE_MSG's exact shape.
explicit_sha=""
range_from=""
range_to=""
if [ "$#" -eq 2 ]; then
	explicit_sha="$1"
	file="$2"
	case "$explicit_sha" in
	*..*)
		range_from="${explicit_sha%%..*}"
		range_to="${explicit_sha##*..}"
		explicit_sha=""
		for r in "$range_from" "$range_to"; do
			git rev-parse --verify --quiet "${r}^{commit}" >/dev/null 2>&1 ||
				{ echo "error: not a commit: $r" >&2; exit 2; }
		done
		;;
	esac
elif [ "$#" -eq 1 ]; then
	...
	echo "usage: ./.fork/conflict-context.sh [<upstream-sha>|<from>..<to>] <file>" >&2
	exit 2
fi
...
if [ -n "$range_to" ]; then
	# A `port.sh range` catch-up: there is no single upstream commit — show the span's
	# net diff for this file, plus the upstream commits in the span that touched it (so
	# you can read the few that matter instead of the squashed wall).
	printf '%s\n' "-- upstream range being ported ($(git rev-parse --short "$range_from")..$(git rev-parse --short "$range_to")) --"
	git diff --stat --patch "$range_from" "$range_to" -- "$file" || true
	printf '%s\n' "" "-- upstream commits in the span touching $file --"
	git log --oneline "$range_from..$range_to" -- "$file" || true
	printf '%s\n' "" "-- your recent local history for $file --"
	# --follow: the fork's history for this file often lives under an OLD name (you
	# renamed it, or you are resolving upstream's rename) — a plain pathspec shows
	# nothing then, exactly when the local intent matters most. brief.sh is already
	# rename-aware; this keeps the two consistent. (--follow takes a single path,
	# which is what this section has.)
	git log --follow --max-count=5 --oneline HEAD -- "$file" || true
elif [ -n "$ported" ]; then
	...
	printf '%s\n' "" "-- your recent local history for $file --"
	# --follow: see the range branch above — local history must survive renames.
	git log --follow --max-count=5 --oneline HEAD -- "$file" || true
