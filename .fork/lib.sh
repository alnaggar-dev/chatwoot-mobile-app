#!/usr/bin/env bash
# .fork/lib.sh — shared helpers for the fork-flow scripts (see Fork Maintenance in your AGENTS.md).
#
...
toolkit_path() {
	case "$1" in
	.fork/* | .fork-flow/* | .omp/skills/* | .gitattributes | AGENTS.md | .github/workflows/fork-flow-gate.yml) return 0 ;;
...
# The customization slug trailer. A `custom:` commit SHOULD carry one
# "Fork-Flow-Change: <slug>" trailer naming the .fork/CHANGES.md entry it belongs
# to, so drop-change/propose-upstream and the port pause banners can find the
# entry's commits exactly (path archaeology is the fallback for pre-trailer
# entries). Advisory, never gating: the commit-msg hook only hints when missing.
CHANGE_TRAILER='Fork-Flow-Change'
# The early-pick trailer. `port.sh pick <sha>` cherry-picks one upstream commit
# OUT OF ORDER (a security fix the backlog can't wait for) WITHOUT advancing the
# pointer; the commit records the exact upstream SHA here. When the contiguous
# catch-up later reaches that SHA, start_port sees the trailer and announces the
# expected empty port — exact-SHA dedupe, no patch-id guessing.
# shellcheck disable=SC2034  # written/grepped by port.sh only
EARLY_TRAILER='Fork-Flow-Early'
...
# The commits linked to registry entry SLUG ($1) via the Fork-Flow-Change trailer,
# newest first, one full SHA per line. The slug is interpolated into a git --grep
# REGEX, so it is restricted to [A-Za-z0-9_-]: any other character (a '.', a space,
# a regex metacharacter) would corrupt the pattern and silently match the wrong
# commits — refuse loudly instead. The `case` lives here (a lib function) per the
# bash-3.2 rule at the top of this file, so callers may use it inside $(...).
change_commits_for_slug() { # <slug>
	case "$1" in
	'' | *[!A-Za-z0-9_-]*)
		echo "change_commits_for_slug: slug must be non-empty [A-Za-z0-9_-] (got: $1)" >&2
		return 1
		;;
	esac
	git log --format='%H' --grep="^${CHANGE_TRAILER}: ${1}\$" HEAD 2>/dev/null
	return 0
}

# The newest IN-EFFECT Fork-Flow-Port[-Init] trailer reachable from HEAD: prints
# "<containing-commit-sha> <trailer-sha>" (trailer SHA resolved via rev-parse when
# possible) on one line, or nothing when no in-effect trailer exists. After a port
# you usually add custom commits, so HEAD is typically NOT a port; checking only
# HEAD would let an ordinary commit advance .fork/UPSTREAM to a bogus SHA unseen.
# A `port.sh revert` (Fork-Flow-Unport) rewinds the pointer, so the reverted
# port's trailer is STALE and must be skipped. Walk newest-first: an Unport(X)
# cancels the next OLDER Port(X) we meet (a re-port, being newer, is seen first
# and wins); the first port/init trailer NOT so cancelled is in effect. The walk
# terminates at that first in-effect trailer — O(commits since the last port) in
# practice. Shared by audit.sh (warn) and port.sh (refuse at port time). Always
# returns 0 so `x="$(effective_port_trailer_sha)"` never trips set -e.
effective_port_trailer_sha() {
	local sha ut pt pending=""
...
registry_lint() {
	...
	while IFS= read -r line || [ -n "$line" ]; do
		...
		"##"*)
			...
			rc=1
			;;
		[Tt]ouches:* | [Ss]ymbols:* | [Vv]erify:* | [Dd]rift-if:* | [Ss]tatus:* | [Tt]est:*)
...
# Which registry entries do the given PATHS (newline list, $1) hit? Prints each
# matching entry heading once, in registry order. Anchors are the entry's
# Touches tokens + the @path part of each Symbols token, glob-aware via
# path_registered — the SAME semantics as verify.sh's scoped gate and brief.sh's
# briefing, so port.sh's pause decision can never disagree with the gate about
# what "touches the registry" means. Entry-aware like every parser here (fields
# count only under a "## " heading). Always returns 0; prints nothing when the
# registry is absent or nothing matches.
registry_entries_for_paths() { # <paths-newline> [changes-file]
	local paths="$1" changes="${2:-.fork/CHANGES.md}" line heading="" touches="" symbols=""
	[ -f "$changes" ] || return 0
	[ -z "$paths" ] && return 0
	_refp_emit() {
		local anchors s p
		...
		if [ -n "$anchors" ]; then
			while IFS= read -r p; do
				[ -z "$p" ] && continue
				if path_registered "$p" "$anchors"; then
					printf '%s\n' "$heading"
					break
				fi
			done <<EOF
...
b.js' ] || { echo 'lib_selftest: registry_paths_stream broken (entry-aware Touches parse)' >&2; bad=1; }
	if change_commits_for_slug 'bad.slug' 2>/dev/null; then
		echo 'lib_selftest: change_commits_for_slug broken (must reject a slug with ".")' >&2
		bad=1
	fi
	if change_commits_for_slug 'bad slug' 2>/dev/null; then
		echo 'lib_selftest: change_commits_for_slug broken (must reject a slug with a space)' >&2
		bad=1
	fi
	type effective_port_trailer_sha >/dev/null 2>&1 || { echo 'lib_selftest: effective_port_trailer_sha missing (port-time pointer gate would go quiet)' >&2; bad=1; }
	return "$bad"
}
